...
%
Les tests d'acceptation sont ex\'ecut\'es \`a l'aide de \texttt{behat}, un \og framework \fg{} PHP d'automatisation de tests qui se base sur le \og \textit{Behavior Driven Development} (BDD) \fg{}. \PG{Je ne retrouve plus mon livre de BDD que j'ai lu...}
\GT{Est-ce @Book{Smart15,
  author =    {Smart, J.F.},
  title =        {{BDD} In Action},
  publisher =    {Manning},
  year =         2015}}
...
La structure des tests d'acceptation avec \texttt{behat} est comme suit (un exemple se trouve dans l'extrait de code~\ref{code:behattest}, p.~\pageref{code:behattest}):
\begin{itemize}
...
\PG{Voir prochain chapitre.}

\section{Tests dans un module d'extension}
Chaque module d'extension d\'efinit ses propres tests unitaires et d'acceptation.
...
Les tests ajout\'es sont pr\'esent\'es \`a la section~\ref{dev_test}.
